...
syst <- function(t,y,parameters, msys){
  ...
  matrix_phi <- matrix(0,nrow = nrow(msys))
  # Remplissage de la matrice 
  for (numReact in seq(nombreReact)){
    ...
    # Pour chaque réactant :
      ...
      r_stochioProd <- c(r_stochioProd , as.numeric(xml_data$model$listOfReactions[[numReact]][[1]][[reactant]][[2]]))
    ...
    if (is.empty(interProd) & is.empty(interSub)){
      ...
      valueCext <- as.numeric(subset(parameters$diffusion,X1==gsub("M_","",nameCext))[[2]] ) #On va chercher la constante dans la table de données
